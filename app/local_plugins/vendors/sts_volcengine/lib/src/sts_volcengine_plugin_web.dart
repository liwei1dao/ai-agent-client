...
/// StsVolcenginePlugin — Volcengine realtime Speech-to-Speech (web implementation).
///
...
class StsVolcenginePlugin implements StsPlugin {
  ...
  // Recognition round state — see [StsPlugin] lifecycle rules in sts_plugin.dart.
  // volcengine 目前只在 chat_ended 给出完整 bot 文本，所以一个回合的
  // recognitionStart/Done/End 会集中派发；ASR 侧的 partial/final 事件暂未
  // surfacing（需要 PM 对齐 volcengine 具体 event code 后再补）。
  ...
  void _handleServerEvent(int event, Uint8List payload, int serType) {
    ...
    switch (event) {
      ...
      case _evtClearAudio:
        ...
        // can be interrupted cleanly. Force-close the current round with
        // `interrupted=true` on playbackEnd.
        _cancelPlayback();
        _forceCloseRound(interrupted: true);
        break;
      case _evtChatEnded:
        ...
        break;
      case _evtTtsEnded:
        // Server signals the bot TTS stream is finished — the browser will
        // finish playing any queued AudioBuffers; from the protocol's POV the
        // round is now complete.
        _forceCloseRound(interrupted: false);
        break;
      case _evtTtsType:
        // TTS type signal — no direct lifecycle mapping.
        break;
      default:
  ...
  /// Called before emitting [StsEventType.audioChunk] — if there's no active
  /// round, open a bot-only one so the audioChunk / playback events still
  /// carry a [StsEvent.requestId] per the protocol.
  ...
  /// Emit an entire bot turn as `recognitionStart → recognized → recognitionDone`
  /// in one shot. volcengine's `_evtChatEnded` surfaces the full utterance
  /// without prior streaming partials.
  void _emitBotTurnWhole(String fullText) {
    ...
    _botRoleOpen = false;
    // Note: recognitionEnd is deferred to playback end (or _forceCloseRound) so
    // audioChunk frames arriving after chat_ended still share the same requestId.
  }

  /// Force-close the current round. Called when the user interrupts (clear
  /// audio) or on teardown. Emits any missing `recognitionDone` + a final
  /// `playbackEnd` and `recognitionEnd`.
