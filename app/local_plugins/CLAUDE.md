...
- 所有接口、配置、事件结构统一定义在 [ai_plugin_interface/](core/ai_plugin_interface/)，厂商包只做**实现**，禁止新增面向业务层的公开类型。
...
local_plugins/
├── core/                      # 接口层 + 跨域基础设施
│   ├── ai_plugin_interface/       # AI 域所有抽象
│   ├── device_plugin_interface/   # 设备域所有抽象
│   └── local_db/                  # 跨域持久化
├── agents/                    # AI 调度 / 容器（核心逻辑层）
│   ├── agent_chat/  agent_sts_chat/
│   ├── agent_translate/  agent_ast_translate/
│   ├── agents_server/             # agent 工厂注册 + DeviceAgentRouter
│   └── service_manager/           # AI 厂商工厂注册
├── vendors/                   # AI 厂商实现（命名 <能力>_<厂商>）
│   ├── llm_openai/  llm_volcengine/
│   ├── stt_azure/  tts_azure/
│   ├── sts_doubao/  sts_polychat/  sts_volcengine/
│   ├── ast_polychat/  ast_volcengine/
│   └── translation_aliyun/  translation_azure/  translation_deepl/
├── devices/                   # 设备域容器 + 厂商 SDK 插件
│   ├── device_manager/            # 单设备路由器
│   └── device_jieli/              # 杰理 RCSP 实现（恒玄/高通/中科蓝讯待补）
├── translate_server/          # 业务服务：复合场景编排（通话/面对面/音视频翻译）
└── assistant_server/          # 业务服务：AI Assistant orchestrator (native + RCSP)
```

新增包入哪一组：
- 接口或跨域工具 → `core/`
- 上层调度（agent/容器） → `agents/`
- 单一 AI 能力的厂商实现 → `vendors/`
- 设备 SDK 厂商或设备协调器 → `devices/`
- 业务服务（多 agent + 设备硬件通道编排，非核心调度层） → `local_plugins/` 顶层
...
| 域 | 接口层 | 厂商实现 | 调度/容器 |
| --- | --- | --- | --- |
| AI 能力 | [ai_plugin_interface/](core/ai_plugin_interface/) | `stt_*` / `tts_*` / `sts_*` / `ast_*` / `llm_*` / `translation_*` | `agent_*` / [service_manager/](agents/service_manager/) / [agents_server/](agents/agents_server/) |
| 硬件设备 | [device_plugin_interface/](core/device_plugin_interface/) | `device_<vendor>`（杰理 / 恒玄 / 高通 / 中科蓝讯 …） | [device_manager/](devices/device_manager/) + `agents_server` 内的 `DeviceAgentRouter` |
...
参见 [ai_plugin_interface/lib/src/stt_plugin.dart](core/ai_plugin_interface/lib/src/stt_plugin.dart)。
...
参见 [ai_plugin_interface/lib/src/tts_plugin.dart](core/ai_plugin_interface/lib/src/tts_plugin.dart)。
...
STS 参见 [ai_plugin_interface/lib/src/sts_plugin.dart](core/ai_plugin_interface/lib/src/sts_plugin.dart)；
AST 参见 [ai_plugin_interface/lib/src/ast_plugin.dart](core/ai_plugin_interface/lib/src/ast_plugin.dart)。
...
STS 是"识别 + 合成 + 播报"的合体，`StsEventType` 覆盖三个生命周期 + 连接 + 音频 + 错误，事件定义见 [sts_plugin.dart](core/ai_plugin_interface/lib/src/sts_plugin.dart)。
...
参见 [ai_plugin_interface/lib/src/llm_plugin.dart](core/ai_plugin_interface/lib/src/llm_plugin.dart)。
...
参见 [ai_plugin_interface/lib/src/translation_plugin.dart](core/ai_plugin_interface/lib/src/translation_plugin.dart)。
...
1. 在 [ai_plugin_interface/](core/ai_plugin_interface/) 找到对应抽象类，**不要**改动接口。如需改动，先提案到本文件 §2 / §3 统一规范。
2. 新建 `<能力>_<厂商>/` 目录，`pubspec.yaml` 声明依赖 `ai_plugin_interface`。
3. 实现类以 `XxxPluginImpl` 命名，在 [service_manager/](agents/service_manager/) 的工厂中注册。
...
参见 [device_plugin_interface/lib/device_plugin_interface.dart](core/device_plugin_interface/lib/device_plugin_interface.dart)、[device_manager/lib/device_manager.dart](devices/device_manager/lib/device_manager.dart)。

### 10.1 分层

- **接口层** [device_plugin_interface/](core/device_plugin_interface/)：`DevicePlugin` / `DeviceSession` / `DeviceManager` 抽象 + `AudioFormat` / `AudioFrame` / `DeviceAgentTrigger` 等数据类型；**唯一**允许被业务层直接依赖的设备域包。
- **厂商实现** `device_<vendor>/`：仅实现 `DevicePlugin` + `DeviceSession`，封装具体 SDK（杰理 RCSP、恒玄 BES SDK、高通 QCC 等）。
- **容器层** [device_manager/](devices/device_manager/)：`DefaultDeviceManager` 单设备路由器；负责厂商工厂注册、热切换、单 active session、音频端口分配、唤醒触发器派发。
...
7. 在 [device_manager/](devices/device_manager/) 的注册位置（业务侧统一启动代码）`registerVendor(yourDescriptor)`。
8. 自测：覆盖 §10.3 状态机、§10.4 音频独占、§10.6 错误码、§10.9 OTA 端口四项。

### 10.9 OTA 端口规范（强制）

参见 [device_plugin_interface/lib/src/device_ota.dart](core/device_plugin_interface/lib/src/device_ota.dart) / Kotlin 同名 `Ota.kt`。**所有声明 `DeviceCapability.ota` 的厂商插件必须实现 [DeviceOtaPort]**，路径与 [DeviceCallTranslationPort] 完全对齐：`DeviceSession.otaPort()` 返回端口，业务层不区分厂商。
...
- 仅依赖 [device_plugin_interface/](core/device_plugin_interface/)；**禁止**依赖 `agents_server` / `agent_*` / `device_jieli` 等具体厂商包。
