    ...
    private fun onPluginEvent(e: DevicePluginEvent) {
        when (e.type) {
            ...
            DevicePluginEventType.DEVICE_DISCOVERED -> {
                // [诊断探针3] plugin.eventStream → manager 这一跳通了，转发到 device_manager/events。
                Log.d(TAG, "DEVICE_DISCOVERED → forward to events channel: " +
                    "id=${e.discovered?.id} name=${e.discovered?.name}")
                emit(DeviceManagerEvent(
