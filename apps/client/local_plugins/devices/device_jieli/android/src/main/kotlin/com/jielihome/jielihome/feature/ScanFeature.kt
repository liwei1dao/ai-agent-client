...
class ScanFeature(
    ...
    private fun handleScanResult(result: ScanResult) {
        ...
        // [诊断探针1] 系统回调进来的每一条原始结果。能看到这行 = scanner 有回调；
        // 完全看不到 = BLE 层没出结果（限流 / 定位服务关 / 设备不广播）。
        Log.d(TAG, "onScanResult addr=${device.address} name=$name rssi=${result.rssi} " +
            "skipUnnamed=$currentSkipUnnamed nameFilter=$currentNameList")

        // 未命名过滤（默认 ON）：drop 掉 name 为空的环境噪声
        if (currentSkipUnnamed && name.isNullOrEmpty()) {
            Log.d(TAG, "  ↳ dropped: unnamed (skipUnnamed=true)")
            return
        }
        ...
        if (names.isNotEmpty()) {
            if (name.isNullOrEmpty() ||
                ...
                Log.d(TAG, "  ↳ dropped: name '$name' not in filter $names")
                return
            }
        }
        ...
        // [诊断探针1b] 通过全部过滤，即将 fan-out 到 dispatcher（Flutter sink + native listener）。
        Log.d(TAG, "  ↳ dispatch deviceFound addr=${device.address} name=$name")
        dispatcher.send(
            mapOf(
                "type" to "deviceFound",
                "name" to (name ?: ""),
